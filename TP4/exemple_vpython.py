from vpython import *
from statistics import * # apelle du module car souvent utilisé
...
# VAriable ajoutées pour l'enregistrement des colissions de l'atome 0
coll_dist = [] # liste des distances libre parcourue entre les collisions pour l'atome 0 
coll_time = [] # liste des temps entre les collisions pour l'atome 0 
coll_count = 0

#### CANVAS DE FOND ####
L = 1 # container is a cube L on a side
gray = color.gray(0.7) # color of edges of container and spheres below
animation = canvas(width=400, height=400)
animation.range = L
...
pavg = sqrt(2*mass*1.5*k*T) # average kinetic energy p**2/(2mass) = (3/2)kT : Principe de l'équipartition de l'énergie en thermodynamique statistique classique
...
while True:
    ...
    ### Ajout de la distance et du temps parcourue, si collison, les valeurs de la liste sont modifié après
    if coll_count>0:
        coll_dist[-1] = coll_dist[-1]+ mag(deltax[0])
        coll_time[-1] = coll_time[-1]+ dt
    ...
    for ij in hitlist:
        ...
        ### Ajout et modification des listes  de collisons avec les informations pertinantes
        if j == 0:
            if coll_count > 0:
                coll_dist[-1]= coll_dist[-1]- mag(vj*deltat) #distance parcourue avant la collision
                coll_time[-1] = coll_time[-1] - deltat
            coll_time.append(deltat)
            coll_dist.append(mag((p[j]/mass)*deltat)) # distance après collision
            coll_count += 1
